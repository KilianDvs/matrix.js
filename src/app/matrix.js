class Matrix {
    ...
    constructor(arr) {
        ...
        if(isArr(arr) && arr[0] !== undefined) { //Handling Arrays

            if(isArr(arr[0])) { //Handling 2D arrays
            ...
            else if(typeof(arr[0]) === "object" && typeof(arr[0]) !== null) { //Handling arrays containing an object
            ...
            else { //Handling 1D arrays

                this.type = "Array"
                this.rows.count = arr.length
                this.cols.count = 1
                this.cols.keys = [0]
                let rowKeys = [], colValues = []
                for(let i in arr) {
        ...
        else if(typeof(arr) === "object" && typeof(arr) !== null) { //Handling objects

            if(isArr(v(arr)[0])) { //Handling objects containing an array

                this.type = "Object of Arrays"
                arr = objToArr(arr)
                this.rows.count = arr.length
                this.cols.count = arr[0][1].length
                let rowKeys = [], colKeys = [], rowValues = []
                for(let i in arr) {
            ...
            else if(typeof(v(arr)[0]) === "object" && typeof(v(arr)[0]) !== null) { //Handling 2D objects
    ...
    insertrows(value, index = undefined, key) { return insertrowsProt(value, index, key, this) }
